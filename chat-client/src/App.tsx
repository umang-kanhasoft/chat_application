...
function App() {
    const { isAuthenticated, currentUserId, currentUserName, initializeAuth, accessToken } =
        useAuthStore();
    const { connect, loadProjects, isConnected, loadGlobalUsers } = useWebSocket();
    ...
    useEffect(() => {
        if (isAuthenticated && status === ConnectionStatus.CONNECTED) {
            loadProjects();
            loadGlobalUsers(); // Load global users by default
        }
    }, [isAuthenticated, status, loadProjects, loadGlobalUsers]);
    ...
    return (
        <>
            ...
            <div className={showConnectionBanner ? 'pt-10' : ''}>
                {/* Navigation Header - Hidden in chat view */}
                    ...
                    className={
                        currentView === 'chat'
                            ...
                            : currentView === 'dashboard' || currentView === 'projects'
                              ? 'pt-16 md:pt-20 min-h-screen bg-slate-50 pb-6 md:pb-0 overflow-auto'
                              : 'min-h-screen bg-white overflow-auto'
                    }
                >
                    <Suspense fallback={<FullScreenLoader />}>
                        <AnimatePresence mode="wait">
                            ...
                            {currentView === 'chat' && (
                                ...
                                >
                                    <ChatLayout onOpenMenu={() => setIsDrawerOpen(true)} />
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </Suspense>
            ...
            {/* Mobile Drawer - Show in chat view too */}
            <MobileDrawer
                isOpen={isDrawerOpen}
